...
def _seg_features(g4: np.ndarray, g5: np.ndarray, vel: np.ndarray) -> np.ndarray:
    ...
    Per-channel (g4, g5) — 15 features each:
        Time-domain (8):
            rms, mean, std, peak, crest_factor, kurtosis, skewness,
            vel_norm_rms (rms / mean_vel — speed-normalised vibration intensity)
        Frequency-domain (7):
            band_energy_5_30hz   - structural/chassis resonance band
            band_energy_30_100hz - wheel/track contact band (incl. 78-86 Hz hot zone)
            band_energy_100_200hz - damping-sensitive band (Damaged -40% at 150-160 Hz)
            band_energy_200_500hz - high-freq material resonance (incl. 416-436 Hz hot zone)
            peak_freq            - frequency of maximum PSD (shifts with material stiffness)
            spectral_centroid    - center-of-mass of the spectrum
            spectral_entropy     - energy spread; concentrated = resonant material
    ...
    Cross-sensor — 5 features:
        ...
        cross_peak_freq     - dominant frequency in cross-power spectrum
        cross_coherence     - g4/g5 coherence at cross_peak_freq

    Kinematic (1):
        mean_vel            - mean train velocity in this segment

    Total: 15*2 + 5 + 1 = 36 features per segment
    """
    feats = []

    mean_vel = float(np.mean(vel)) if len(vel) > 0 else 1e-6

    # Frequency bands chosen from ANOVA analysis (diagnose_frequencies.py)
    BANDS = [(5, 30), (30, 100), (100, 200), (200, 500)]

    def _spectral_feats(sig: np.ndarray) -> list:
        n = len(sig)
        nperseg = min(n, 64)
        if nperseg < 8:
        ...
        # Normalised band energies
        band_e = []
        for flo, fhi in BANDS:
            mask = (f >= flo) & (f <= fhi)
            band_e.append(float(pxx[mask].sum() / total))

        # Peak frequency (shifts with material stiffness)
        peak_f = float(f[np.argmax(pxx)])

        # Spectral centroid
        centroid = float(np.dot(f, pxx) / total)

        # Spectral entropy (normalised by log(N) so it is in [0, 1])
        p_norm = pxx / total
        entropy = float(-np.sum(p_norm * np.log(p_norm + 1e-30)) / np.log(len(f) + 1))

        return band_e + [peak_f, centroid, entropy]
    ...
    for sig in (g4, g5):
        ...
        skw   = float(skew(sig))
        vnrms = rms / (mean_vel + 1e-12)
        feats.extend([rms, mean_, std, peak, crest, kurt, skw, vnrms])
        feats.extend(_spectral_feats(sig))

    # Cross-sensor time-domain
    ...
    # Cross-spectral: shared dominant frequency + coherence
    n = min(len(g4), len(g5))
    nperseg = min(n, 64)
    if nperseg >= 8:
        f_c, pxy    = csd(g4[:n], g5[:n], fs=FS, nperseg=nperseg)
        peak_idx    = int(np.argmax(np.abs(pxy)))
        cross_peak  = float(f_c[peak_idx])
        _, coh_arr  = coherence(g4[:n], g5[:n], fs=FS, nperseg=nperseg)
        cross_coh   = float(coh_arr[peak_idx])
    else:
        cross_peak = 0.0
        cross_coh  = 0.0
    feats.extend([cross_peak, cross_coh])
...
def feature_names() -> list[str]:
    per_seg = []
    for ch in ("g4", "g5"):
        for stat in ("rms", "mean", "std", "peak", "crest", "kurtosis", "skewness",
                     "vel_norm_rms",
                     "band_5_30hz", "band_30_100hz", "band_100_200hz", "band_200_500hz",
                     "peak_freq", "spectral_centroid", "spectral_entropy"):
            per_seg.append(f"{ch}_{stat}")
    per_seg += ["cross_corr", "cross_rms_ratio", "cross_diff_rms",
                "cross_peak_freq", "cross_coherence", "mean_vel"]
...
def run_pipeline():
    ...
    n_feats = X.shape[1]
    feats_per_seg = n_feats // N_SEGMENTS
    print(f"\nDataset: {X.shape[0]} runs x {n_feats} features "
          f"({N_SEGMENTS} segments x {feats_per_seg} features/segment)")
